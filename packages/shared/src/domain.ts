...
export function normalizeErrorTags(input: {
  ...
  const secondary = [...new Set(input.secondary ?? [])]
    .filter((tag) => tag !== main)
    .slice(0, 2);
...
export function initialMistakeStatus(ocrText: string): MasteryStatus {
  return ocrText.trim().length > 0 ? "pending_analysis" : "pending_analysis";
}
